...
- `shared/agent-skills.nix` validates the shared skill graph, installs the full
  bundle into Claude, and installs a smaller Codex bundle to avoid Codex
  startup skill-context budget truncation
...
- both receive installed skills from `shared/agent-skills.nix`; Claude gets the
  full bundle, while Codex gets a smaller runtime bundle from the same source
  graph to stay within Codex skill-context budget
