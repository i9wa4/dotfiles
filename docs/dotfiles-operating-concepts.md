...
- `shared/agent-skills.nix` installs both local and upstream skills into both
  engines
- `subagents/*.md` is the native reviewer Markdown source of truth;
...
- both receive native reviewer agents from `subagents/*.md`, with Codex
...
`subagents/*.md` defines native reviewer agents as Markdown:
