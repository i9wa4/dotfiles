---
title: "Unit testing in dbt for test-driven development | dbt Developer Blog"
source_url: "https://docs.getdbt.com/blog/announcing-unit-testing"
...
## Story of data quality in dbt[​](#story-of-data-quality-in-dbt "Direct link to Story of data quality in dbt")
...
## Introducing unit testing in dbt[​](#introducing-unit-testing-in-dbt "Direct link to Introducing unit testing in dbt")
...
## Hello, unit testing world[​](#hello-unit-testing-world "Direct link to Hello, unit testing world")

[![Hello unit testing world](https://docs.getdbt.com/img/blog/2024-05-07-unit-testing/hello-world.png?v=2 "Hello unit testing world")](#)Hello unit testing world
...
[![Terminal output of hello world unit test](https://docs.getdbt.com/img/blog/2024-05-07-unit-testing/unit-test-terminal-output.png?v=2 "Terminal output of hello world unit test")](#)Terminal output of hello world unit test

Voilà! We can see that a single unit test ran and it passed.

## Crafting Unit Tests[​](#crafting-unit-tests "Direct link to Crafting Unit Tests")
...
### Organizing your thoughts[​](#organizing-your-thoughts "Direct link to Organizing your thoughts")
...
### Writing your unit tests[​](#writing-your-unit-tests "Direct link to Writing your unit tests")
...
To implement the unit test, we started by starting with a "model-inputs-output" (MIO) structure above. The relevant **model** was `orders` with **given** inputs were from `order_items` and `stg_orders`. In this case, we **expect** our output for order\_id 2 to be `count_drink_items: 0`.

Here's what the unit test YAML looked like:

### Unit test YAML[​](#unit-test-yaml "Direct link to Unit test YAML")
...
### Original SQL code[​](#original-sql-code "Direct link to Original SQL code")
...
### SQL Code fix[​](#sql-code-fix "Direct link to SQL Code fix")

You can see that we changed `count` to `sum` in the SQL code:

[![SQL changes in code](https://docs.getdbt.com/img/blog/2024-05-07-unit-testing/changed_code.png?v=2 "SQL changes in code")](#)SQL changes in code

### Caveats and pro-tips[​](#caveats-and-pro-tips "Direct link to Caveats and pro-tips")
...
## Unit tests vs. model contracts vs. data tests[​](#unit-tests-vs-model-contracts-vs-data-tests "Direct link to Unit tests vs. model contracts vs. data tests")
...
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Unit tests Model contracts Data tests|  |  |  |  |  |  |  |  |  |  |  |  | | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | | Enforced before a resource node is materialized Enforced while the resource node is materialized Enforced after a resource node is materialized|  |  |  |  |  |  |  |  |  | | --- | --- | --- | --- | --- | --- | --- | --- | --- | | Blocks the attempt to build the resource Blocks the building the resource node and downstream nodes Blocks building of downstream nodes|  |  |  |  |  |  | | --- | --- | --- | --- | --- | --- | | Rigid tests of the exact expected output for a single transformation Tests the "shape" of the container (column names and data types) for a single data set Flexible and can test assertions across multiple data sets, ranges of values, etc.|  |  |  | | --- | --- | --- | | Good for testing the precise values expected in the output Good for enforcing the column names and data types that describe the "shape" of the data and specifying constraints like primary and foreign keys Good for testing assertions other than equality (like ranges of acceptable values) or source data whose transformation is a black box | | | | | | | | | | | | | | |
...
Up and Running with Azure Synapse on dbt Cloud](https://docs.getdbt.com/blog/synapse-best-practices)[Older post

Conversational Analytics: A Natural Language Interface to your Snowflake Data](https://docs.getdbt.com/blog/semantic-layer-cortex)
