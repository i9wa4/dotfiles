...
printf '  %-29s = %s\n' "Codex history/config" "review_first; lightweight history stays separate from session-rollout retention."
printf '  %-29s = %s\n' "Codex sessions" "review_first; keep about 50 days of closed rollout JSONL by file age, skip live open sessions, and preserve ccusage-codex input."
printf '  %-29s = %s\n' "Codex TUI log" "review_first; codex-tui.log stays outside the session-rollout retention rule."
printf '  %-29s = %s\n' "tmux-a2a-postman state" "review_first; control-plane state needs manual review, not age-based prune."
printf '  %-29s = %s\n' "vde-monitor state" "review_first; startup prunes only ~/.vde-monitor/panes while durable state stays preserved."
printf '  %-29s = %s\n' "tool payloads" "review_first; payloads outside the Nix store may back active tools."
printf '  %-29s = %s\n' "repo-local rebuildables" "review_first; decide per repo instead of host-wide cache cleanup."
