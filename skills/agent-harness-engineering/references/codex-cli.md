...
| Codex agents directory   | TOML generated from `subagents/claude/*.md` Markdown      | `shared/install-manifest.nix`        |
| Codex rules directory    | Generated `default.rules` from denied Bash policy         | `codex/default.nix`                  |
...
prefix convention. Scripts are materialized into the Codex scripts directory at
switch time.
...
- [x] Subagent support - multi-agent feature stabilized (v0.115.0); TOML agents
  are generated from `nix/home-manager/agents/subagents/claude/*.md` by
