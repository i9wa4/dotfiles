...
def get_context(text: str, query: str, context_lines: int = 2) -> List[str]:
    ...
    match_indices = [
        i for i, line in enumerate(lines) if any(kw in line.lower() for kw in keywords)
    ]
    ...
    for group in groups:
        ...
        # In a real terminal, we could use ANSI codes,
        # but for text output we keep it clean.
        # We add a marker like '> ' for matched lines.
...
def main():
    ...
    parser.add_argument(
        "--max-results", "-n", type=int, default=10, help="Maximum number of results"
    )
    ...
    parser.add_argument(
        ...
        help="Skill directory (default: auto-detected from script location)",
    )
