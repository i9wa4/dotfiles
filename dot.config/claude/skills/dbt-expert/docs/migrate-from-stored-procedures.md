---
title: "Migrate from DDL, DML, and stored procedures | dbt Developer Hub"
source_url: "https://docs.getdbt.com/guides/migrate-from-stored-procedures"
...
Ask questions about this page](https://chatgpt.com/?hints=search&prompt=Read+from+https%3A%2F%2Fdocs.getdbt.com%2Fguides%2Fmigrate-from-stored-procedures+so+I+can+ask+questions+about+it.)[Open in Claude

Ask questions about this page](https://claude.ai/new?q=Read+from+https%3A%2F%2Fdocs.getdbt.com%2Fguides%2Fmigrate-from-stored-procedures+so+I+can+ask+questions+about+it.)[Open in Perplexity

Ask questions about this page](https://www.perplexity.ai/search/new?q=Read+from+https%3A%2F%2Fdocs.getdbt.com%2Fguides%2Fmigrate-from-stored-procedures+so+I+can+ask+questions+about+it.)
...
Converting this with a first pass to a [dbt model](https://docs.getdbt.com/guides/bigquery?step=8) (in a file called returned\_orders.sql) might look something like:
...
* `incremental_strategy` here is set to MERGE any existing rows in the target table with a value for the `unique_key` which matches the incoming batch of data. There are [various incremental strategies](https://docs.getdbt.com/docs/build/incremental-strategy) for different situations and warehouses.
