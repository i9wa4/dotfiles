...
- [x] `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` left unset (2026-09-19; was explicit
  ...
  the harness never even reached them. Leaving it unset matches the
  documented default (no scrubbing, same behavior as the prior explicit
  `"0"`) and removes a redundant setting without touching the "1" trigger
  value that caused the incident. Restore to "1" only after declaring an
