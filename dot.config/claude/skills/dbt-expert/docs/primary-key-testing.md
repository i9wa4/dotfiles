---
title: "What's a Primary Key and Why Do We Test Them? | dbt Developer Blog"
source_url: "https://docs.getdbt.com/blog/primary-key-testing"
...
## What’s a primary key?[​](#whats-a-primary-key "Direct link to What’s a primary key?")

A primary key is a column in your database that exists to uniquely identify a single row.

Primary keys are *critical* to data modeling. Without a primary key, you’ll find yourself constantly struggling to identify duplicate rows and define the expected grain of your tables.

There is just about no more ironclad law in the land of analytics than `you must have a primary key on every table`.

## Why do we test primary keys?[​](#why-do-we-test-primary-keys "Direct link to Why do we test primary keys?")
...
Invalid data getting into your primary keys is one of *the* biggest data problems - it can lead to rows getting dropped or miscounted and for all kinds of weird results to pop into your data. It’s one of the most common causes of time-sensitive headaches in analytic-land.
...
## How to test primary keys with dbt[​](#how-to-test-primary-keys-with-dbt "Direct link to How to test primary keys with dbt")

Today, you can add two simple [dbt tests](https://docs.getdbt.com/docs/build/data-tests) onto your primary keys and feel secure that you are going to catch the vast majority of problems in your data.
...
* [Not\_null](https://docs.getdbt.com/reference/resource-properties/tests#not_null)
...
## Does your warehouse support primary keys?[​](#does-your-warehouse-support-primary-keys "Direct link to Does your warehouse support primary keys?")

Does your warehouse even *support* primary keys at all? If it does, how can you actually find out if a table has a primary key set, and what that primary key is?

Let’s walk through primary key support + access across the major cloud data warehouse platforms.

### TL;DR on primary key support across warehouses[​](#tldr-on-primary-key-support-across-warehouses "Direct link to TL;DR on primary key support across warehouses")
...
This means that across major analytics warehouses, *data testing* (using a tool like dbt) to enforce your primary keys is super important for ensuring analytics data quality.
...
### BigQuery primary keys[​](#bigquery-primary-keys "Direct link to BigQuery primary keys")
...
### Databricks primary keys[​](#databricks-primary-keys "Direct link to Databricks primary keys")
...
### Redshift primary keys[​](#redshift-primary-keys "Direct link to Redshift primary keys")
...
### Snowflake primary keys[​](#snowflake-primary-keys "Direct link to Snowflake primary keys")
...
### Postgres primary keys[​](#postgres-primary-keys "Direct link to Postgres primary keys")
...
## Have you started testing primary keys yet?[​](#have-you-started-testing-primary-keys-yet "Direct link to Have you started testing primary keys yet?")
...
The Exact dbt Commands We Run in Production](https://docs.getdbt.com/blog/dbt-production-commands)[Older post
