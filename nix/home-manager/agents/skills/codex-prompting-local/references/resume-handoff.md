...
Use these patterns for long-running Codex tasks, follow-up turns on the same
thread, or result summaries that another agent may need to resume later.

This repo already saves lightweight Codex handoff context through:
...
- If a Codex thread ID is known, surface it explicitly.
...
verification: git diff --check clean; skill files present under nix/home-manager/agents/skills/codex-prompting-local
resume command: codex resume abc123
```
...
```xml
<task>
Resume the previous Codex thread for this repo and continue from the verified
state below.
</task>
