...
in
{
  ...
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      set-option -g status-left " [#(tmux list-sessions -F '##{session_created} ##{session_id}' | awk '{ print $1, substr($2, 2), $2 }' | sort -k1,1n -k2,2n | awk -v current='#{session_id}' '$3 == current { print NR - 1; exit }') #{session_name:50}] "
