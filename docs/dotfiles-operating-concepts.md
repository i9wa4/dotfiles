...
- `shared/agent-skills.nix` installs both local and upstream skills into both
  engines
- `subagents/claude/*.md` is the native reviewer Markdown source of truth;
  `shared/install-manifest.nix` installs it for Claude and generates Codex TOML
  for the Codex agents runtime directory
...
- both receive native reviewer agents from `subagents/claude/*.md`, with Codex
  TOML generated by `shared/install-manifest.nix`
...
`subagents/claude/*.md` defines native reviewer agents as Markdown:

- reviewer agent definitions for Claude are installed directly
