...
- In Neovim, `:R` or `:R {register}` can push a register manually.
...
- Use normal yanks such as `yy` for the default path. In Neovim, use `:R` or
  `:R {register}` when a register must be pushed manually.
