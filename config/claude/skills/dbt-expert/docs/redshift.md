---
title: "Quickstart for dbt and Redshift | dbt Developer Hub"
source_url: "https://docs.getdbt.com/guides/redshift"
...
Ask questions about this page](https://chatgpt.com/?hints=search&prompt=Read+from+https%3A%2F%2Fdocs.getdbt.com%2Fguides%2Fredshift+so+I+can+ask+questions+about+it.)[Open in Claude

Ask questions about this page](https://claude.ai/new?q=Read+from+https%3A%2F%2Fdocs.getdbt.com%2Fguides%2Fredshift+so+I+can+ask+questions+about+it.)[Open in Perplexity

Ask questions about this page](https://www.perplexity.ai/search/new?q=Read+from+https%3A%2F%2Fdocs.getdbt.com%2Fguides%2Fredshift+so+I+can+ask+questions+about+it.)
...
To avoid connectivity issues with dbt, make sure to allow inbound traffic on port 5439 from [dbt's IP addresses](https://docs.getdbt.com/docs/cloud/about-cloud/access-regions-ip-addresses) in your Redshift security groups and Network Access Control Lists (NACLs) settings.
...
3. Click **Next** for each page until you reach the **Select acknowledgement** checkbox. Select **I acknowledge that AWS CloudFormation might create IAM resources with custom names** and click **Create Stack**. You should land on the stack page with a CREATE\_IN\_PROGRESS status.
   ...
   [![Cloud Formation in Progress](https://docs.getdbt.com/img/redshift_tutorial/images/cloud_formation_in_progress.png?v=2 "Cloud Formation in Progress")](#)Cloud Formation in Progress
...
5. Type `Redshift` in the search bar at the top and click **Amazon Redshift**.
   ...
   [![Click on Redshift](https://docs.getdbt.com/img/redshift_tutorial/images/go_to_redshift.png?v=2 "Click on Redshift")](#)Click on Redshift
...
[![Available Redshift Cluster](https://docs.getdbt.com/img/redshift_tutorial/images/cluster_overview.png?v=2 "Available Redshift Cluster")](#)Available Redshift Cluster
...
[![Redshift Query Editor v2](https://docs.getdbt.com/img/redshift_tutorial/images/redshift_query_editor.png?v=2 "Redshift Query Editor v2")](#)Redshift Query Editor v2

[![Connect to Redshift Cluster](https://docs.getdbt.com/img/redshift_tutorial/images/connect_to_redshift_cluster.png?v=2 "Connect to Redshift Cluster")](#)Connect to Redshift Cluster
...
[![Go to S3](https://docs.getdbt.com/img/redshift_tutorial/images/go_to_S3.png?v=2 "Go to S3")](#)Go to S3
...
[![Go to your S3 Bucket](https://docs.getdbt.com/img/redshift_tutorial/images/s3_bucket.png?v=2 "Go to your S3 Bucket")](#)Go to your S3 Bucket

4. Click **Upload**. Drag the three files into the UI and click the **Upload** button.

[![Upload your CSVs](https://docs.getdbt.com/img/redshift_tutorial/images/upload_csv.png?v=2 "Upload your CSVs")](#)Upload your CSVs
...
1. Create a new project in [dbt](https://docs.getdbt.com/docs/cloud/about-cloud/access-regions-ip-addresses). Navigate to **Account settings** (by clicking on your account name in the left side menu), and click **+ New Project**.
...
5. Enter your Redshift settings. Reference your credentials you saved from the CloudFormation template.
   ...
   * **Database** (under **Optional settings**) — `dbtworkshop`
   ...
   [![dbt - Redshift Cluster Settings](https://docs.getdbt.com/img/redshift_tutorial/images/dbt_cloud_redshift_account_settings.png?v=2 "dbt - Redshift Cluster Settings")](#)dbt - Redshift Cluster Settings
   ...
   * Allow inbound traffic on port `5439` from [dbt's IP addresses](https://docs.getdbt.com/docs/cloud/about-cloud/access-regions-ip-addresses) in your Redshift security groups and Network Access Control Lists settings.
...
10. Set your development credentials. These credentials will be used by dbt to connect to Redshift. Those credentials (as provided in your CloudFormation output) will be:
    ...
    * **Schema** — dbt automatically generates a schema name for you. By convention, this is `dbt_<first-initial><last-name>`. This is the schema connected directly to your development environment, and it's where your models will be built when running dbt within the Studio IDE.
    ...
    [![dbt - Redshift Development Credentials](https://docs.getdbt.com/img/redshift_tutorial/images/dbt_cloud_redshift_development_credentials.png?v=2 "dbt - Redshift Development Credentials")](#)dbt - Redshift Development Credentials
