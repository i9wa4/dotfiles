---
title: "The case against `git cherry pick`: Recommended branching strategy for multi-environment dbt projects | dbt Developer Blog"
source_url: "https://docs.getdbt.com/blog/the-case-against-git-cherry-picking"
...
Don't over-engineer your branching strategy
...
![Multi-environment workflow](https://docs.getdbt.com/assets/images/2_multienvironment-6b12b8c58e1eec53b252dcdc5e436a3d.png)
...
![Gru meets cherry picking](https://docs.getdbt.com/assets/images/3_gru-6fb3f766effc6e286253f58bc14bc5dc.jpg)

## If you’re not testing changes independently, you shouldn’t promote them independently[​](#if-youre-not-testing-changes-independently-you-shouldnt-promote-them-independently "Direct link to If you’re not testing changes independently, you shouldn’t promote them independently")
...
![Feature branches tested in combination with others](https://docs.getdbt.com/assets/images/4_scenario_1-7b7d2ce7deb4a1145f810c88df2d900e.png)
...
## Feature branches contain more than meets the eye[​](#feature-branches-contain-more-than-meets-the-eye "Direct link to Feature branches contain more than meets the eye")

Let’s imagine another version of the story, where Carol’s changes are the only ones that are ultimately merged into `prod`:

![Feature branches contain more than meets the eye...](https://docs.getdbt.com/assets/images/5_scenario_2-d4ff4df417d684fd36174c580def9498.png)
...
## Repeated merge conflicts drain development time[​](#repeated-merge-conflicts-drain-development-time "Direct link to Repeated merge conflicts drain development time")
...
![Development branches made from prod](https://docs.getdbt.com/assets/images/6_scenario_3-6df28289d93134e68d570cff143dd2ba.png)

1. Alex wants to make a code change, so they create a new branch from `prod` called `feature_alex`.
2. Becca has a different code change she’s working on, so she creates a new branch from `prod` called `feature_becca`.
3. Alex’s changes are approved, so they merge `feature_alex` into `dev`.
4. Becca’s changes are approved, so she merges `feature_becca` into `dev`.
5. Carol is working on something else, so she creates a new branch from `prod` called `feature_carol`.
6. Carol’s changes are approved, so she merges `feature_carol` into `dev`.
7. The testing team approves the entire `dev` branch.
8. `dev` is merged into `staging`.
9. During final checks, the team notices an issue with Alex and Becca’s changes in `staging`.
...
## What to do instead: The recommended branching strategy for multi-environment dbt projects[​](#what-to-do-instead-the-recommended-branching-strategy-for-multi-environment-dbt-projects "Direct link to What to do instead: The recommended branching strategy for multi-environment dbt projects")

![Bernie is begging you to stop cherry picking](https://docs.getdbt.com/assets/images/7_bernie-f5c78c4b3be581769022f1d4cd9c9070.jpg)
...
![Hierarchical branching strategy](https://docs.getdbt.com/assets/images/2_multienvironment-6b12b8c58e1eec53b252dcdc5e436a3d.png)
...
Analysts make the best analytics engineers](https://docs.getdbt.com/blog/analysts-make-the-best-aes)[Older post

KonMari your data: Planning a query migration using the Marie Kondo method](https://docs.getdbt.com/blog/marie-kondo-query-migration)
